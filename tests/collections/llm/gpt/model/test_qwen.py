...
def test_qwen3_config():
    config = Qwen3Config(num_layers=24, hidden_size=896, num_attention_heads=14)
    assert config.normalization == "RMSNorm"
    assert config.activation_func == F.silu
    assert config.gated_linear_unit is True
    assert config.add_bias_linear is False
...
def test_qwen3_config_600m():
    config = Qwen3Config600M()
    assert config.num_layers == 28
    assert config.hidden_size == 896
    assert config.num_attention_heads == 14
    assert config.num_query_groups == 2
...
def test_qwen3_config_1p7b():
    config = Qwen3Config1P7B()
    assert config.num_layers == 28
    assert config.hidden_size == 1536
    assert config.num_attention_heads == 12
    assert config.num_query_groups == 2
...
def test_qwen3_config_4b():
    config = Qwen3Config4B()
...
def test_qwen3_config_8b():
    config = Qwen3Config8B()
    assert config.num_layers == 32
...
def test_qwen3_config_14b():
    config = Qwen3Config14B()
    assert config.num_layers == 40
    assert config.hidden_size == 5120
    assert config.num_attention_heads == 40
    assert config.num_query_groups == 5
    assert config.ffn_hidden_size == 17920


def test_qwen3_config_32b():
    config = Qwen3Config32B()
